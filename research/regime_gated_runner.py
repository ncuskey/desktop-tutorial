...
def apply_lookback_state_filter(df: pd.DataFrame, params: dict[str, Any]) -> tuple[pd.Series, pd.DataFrame]:
    ...
    atr_expansion_recent_window = int(params.get("atr_expansion_recent_window", 5))
    compression_window = int(params.get("compression_window", 20))

    # Relaxed defaults for R1.2.2: keep sequential logic, avoid trade starvation.
    adx_slope_threshold = float(params.get("adx_slope_threshold", -0.2))
    atr_expansion_threshold = float(params.get("atr_expansion_threshold", 1.0))
    low_vol_percentile = float(params.get("low_vol_percentile", 0.3))
    compression_threshold = float(params.get("compression_threshold", 0.05))
    ...
    atr_expansion_recent = atr_ratio.rolling(
        atr_expansion_recent_window,
        min_periods=max(2, atr_expansion_recent_window // 2),
    ).max()
            ...
            state_diag_rows.append(
                {
                    ...
                    "atr_expansion_recent_mean": _safe_float(
                        state_diag["atr_expansion_recent"].mean(), default=np.nan
                    ),
